...
import jxmlease
import re
from gds.myXML import tokenXML, sabreCommand, readItin
...
class Pnr(object):
    ...
    def send_sabre_entry(self, token, command):
        ...
        url = "https://webservices3.sabre.com"
        headers = {'content-type': 'text/xml'}
        body = sabreCommand % (token, command)
    ...
    def list_of_pnr_in_q(self,q):
        ...
        entry = 'Q/'+str(q)+'/L'
        resp = Pnr().send_sabre_entry(token, entry)
        line = resp.decode().split('\n') #previous line = resp.split('\n')
        x = []
